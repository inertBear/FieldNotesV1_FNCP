...
public class FNLogin extends FNPanel {
    ...
    public void init() {
        ...
        result = action.SQLRegister(productKey);

        if (!result) {
            FNRegister.getInstance();
        } else {
            // validation success, prompt login
            ...
            mButtonLogin.addActionListener(e -> {
                ...
                params.add(new BasicNameValuePair("customerKey", "1159616266"));

                // make HTTP connection
                JSONObject json = mJsonParser.createHttpRequest(postUrl, "POST", params);
